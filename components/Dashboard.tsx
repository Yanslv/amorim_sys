...
const StatCard = ({ title, value, icon: Icon, color, subValue }: any) => (
  <div className="bg-white p-4 sm:p-6 rounded-2xl border border-gray-100 shadow-sm flex items-start justify-between">
    <div className="flex-1 min-w-0">
      ...
      {subValue && <p className="text-xs sm:text-sm text-gray-400 mt-1 truncate">{subValue}</p>}
    </div>
    ...
    <div className="space-y-8 relative">
      <header className="flex flex-col sm:flex-row sm:justify-between sm:items-end gap-4">
        <div>
          ...
          <p className="text-gray-500 mt-2 font-medium text-sm sm:text-base">Bem-vindo de volta! Priorize suas ações imediatas.</p>
        </div>
        <div className="flex space-x-3">
          <div className="bg-white border border-gray-200 px-3 sm:px-4 py-2 rounded-lg flex items-center space-x-2">
            <CalendarIcon size={16} className="text-gray-400" />
            <span className="text-xs sm:text-sm font-semibold">{new Date().toLocaleDateString('pt-BR', { day: '2-digit', month: 'long', year: 'numeric' })}</span>
          </div>
        </div>
      </header>

      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6 lg:gap-8">
        <div className="bg-white p-4 sm:p-6 lg:p-8 rounded-2xl border border-gray-100 shadow-sm text-gray-900">
          <h3 className="text-lg sm:text-xl font-bold mb-4 sm:mb-6">Executar Agora</h3>
              ...
              return (
                ...
                >
                  <div className="flex items-center space-x-3 sm:space-x-4 flex-1 min-w-0">
                    <div className={`p-2 rounded-lg flex-shrink-0 ${task.priority === 'high' ? 'bg-rose-100 text-rose-600' : 'bg-indigo-100 text-indigo-600'}`}>
                      <Clock size={16} className="sm:w-[18px] sm:h-[18px]" />
                    </div>
                    <div className="flex-1 min-w-0">
                      <h4 className="font-bold text-gray-800 text-sm sm:text-base truncate">{task.title}</h4>
                  ...
                  <div className="flex items-center space-x-2 flex-shrink-0">
                    <span className="hidden sm:inline text-[10px] font-bold text-gray-400 opacity-0 group-hover:opacity-100 transition-opacity uppercase">Ver detalhes</span>
        ...
        <div className="bg-white p-4 sm:p-6 lg:p-8 rounded-2xl border border-gray-100 shadow-sm text-gray-900">
          <h3 className="text-lg sm:text-xl font-bold mb-4 sm:mb-6">Próximas Reuniões</h3>
          <div className="space-y-3 sm:space-y-4">
            {meetings.length === 0 ? (
      ...
      {selectedTaskDetail && (
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-50 flex items-center justify-center p-2 sm:p-4 overflow-y-auto">
          <div className="bg-white rounded-2xl sm:rounded-3xl w-full max-w-4xl shadow-2xl animate-in fade-in zoom-in duration-300 overflow-hidden flex flex-col my-auto max-h-[95vh] sm:max-h-[85vh]">
            ...
            <div className="flex-1 overflow-y-auto grid grid-cols-1 md:grid-cols-5 min-h-0">
              ...
              <div className="md:col-span-2 p-4 sm:p-6 lg:p-8 border-r border-gray-100 space-y-4 sm:space-y-6 bg-white">
                <form onSubmit={handleStatusUpdate} className="space-y-4 sm:space-y-6">
                  <h3 className="text-xs sm:text-sm font-black uppercase text-gray-400 tracking-wider">Ações Imediatas</h3>
                  <div className="space-y-3 sm:space-y-4">
                    <div>
              ...
              <div className="md:col-span-3 p-4 sm:p-6 lg:p-8 bg-gray-50 flex flex-col border-t md:border-t-0 md:border-l border-gray-100 min-h-0">
                <h3 className="text-xs sm:text-sm font-black uppercase text-gray-400 tracking-wider flex items-center space-x-2 mb-4 sm:mb-8 flex-shrink-0">
                  <History size={14} className="sm:w-4 sm:h-4" />
                  <span>Timeline de Execução</span>
                </h3>
                <div className="flex-1 space-y-4 sm:space-y-6 overflow-y-auto min-h-0">
