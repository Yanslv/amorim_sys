    ...
    <div className="space-y-6 h-full flex flex-col">
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        ...
        <div className="flex items-center space-x-4">
          <div className="flex bg-white border border-gray-200 rounded-xl overflow-hidden shadow-sm">
            <button onClick={prevWeek} className="p-3 hover:bg-gray-50 border-r border-gray-100"><ChevronLeft size={20}/></button>
            <div className="px-6 py-3 font-black text-sm flex items-center min-w-[180px] justify-center">
              {days[0].toLocaleDateString('pt-BR', { day: '2-digit', month: 'short' })} - {days[6].toLocaleDateString('pt-BR', { day: '2-digit', month: 'short' })}
            </div>
            <button onClick={nextWeek} className="p-3 hover:bg-gray-50 border-l border-gray-100"><ChevronRight size={20}/></button>
          </div>
          <button 
            onClick={() => navigate('/projects')}
            className="bg-indigo-600 text-white px-6 py-3 rounded-xl font-bold flex items-center space-x-2 shadow-lg hover:bg-indigo-700 transition-all"
          >
            <Plus size={20} />
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-white p-4 rounded-2xl border border-gray-100 shadow-sm flex items-center space-x-4">
          ...
          <div>
            <p className="text-[10px] font-black text-gray-400 uppercase">Horas Planejadas</p>
            <p className="text-xl font-black">{stats.planned}h</p>
          </div>
        </div>
        <div className="bg-white p-4 rounded-2xl border border-gray-100 shadow-sm flex items-center space-x-4">
          <div className="bg-emerald-50 p-2 rounded-lg text-emerald-600"><Clock size={20}/></div>
          <div>
            <p className="text-[10px] font-black text-gray-400 uppercase">Horas Livres</p>
            <p className="text-xl font-black">{stats.available}h</p>
          </div>
        </div>
        <div className="bg-white p-4 rounded-2xl border border-gray-100 shadow-sm flex items-center space-x-4">
          <div className="bg-rose-50 p-2 rounded-lg text-rose-600"><AlertCircle size={20}/></div>
          <div>
            <p className="text-[10px] font-black text-gray-400 uppercase">Atrasos</p>
            <p className="text-xl font-black">{stats.delayed}</p>
          </div>
        </div>
        <div className="bg-white p-4 rounded-2xl border border-gray-100 shadow-sm flex items-center space-x-4">
          <div className="bg-amber-50 p-2 rounded-lg text-amber-600"><AlertCircle size={20}/></div>
          <div>
            <p className="text-[10px] font-black text-gray-400 uppercase">Conflitos</p>
            <p className="text-xl font-black">{stats.conflicts}</p>
          </div>
        </div>
      </div>
      ...
      <div className="flex-1 bg-white rounded-3xl border border-gray-100 shadow-xl overflow-hidden flex flex-col">
        <div className="calendar-grid border-b border-gray-100">
        ...
        <div className="flex-1 overflow-y-auto">
          <div className="calendar-grid min-h-[800px]">
            {/* Hours Column */}
            <div className="border-r border-gray-100 bg-gray-50 flex flex-col">
              {hours.map(h => (
                <div key={h} className="h-20 p-2 text-[10px] font-black text-gray-400 text-right border-b border-gray-100">
              ...
              return (
                <div key={i} className="relative border-r border-gray-100 last:border-r-0 group">
                  ...
                  {dayTasks.map(task => {
                    ...
                    const duration = task.estimatedHours * 80;
                    
                    return (
                      <div 
                        key={task.id} 
                        onClick={() => navigate(`/projects/${task.projectId}`)}
                        className={`absolute left-1 right-1 rounded-lg p-2 text-white shadow-md border-l-4 overflow-hidden group cursor-pointer hover:scale-[1.02] transition-transform ${
                      ...
                      >
                        <p className="text-[10px] font-black opacity-80 truncate">{projects.find((p: any) => p.id === task.projectId)?.name}</p>
                        <p className="text-xs font-bold leading-tight mt-0.5 line-clamp-2">{task.title}</p>
                  ...
                  {dayMeetings.map(meeting => {
                    ...
                    return (
                      <div 
                        key={meeting.id} 
                        className="absolute left-1 right-1 bg-amber-500 border-l-4 border-amber-700 rounded-lg p-2 text-white shadow-md h-20"
                        style={{ top: `${top}px` }}
                      >
                        <p className="text-[10px] font-black opacity-80">REUNIÃO</p>
                        <p className="text-xs font-bold leading-tight mt-0.5">{meeting.title}</p>
                      </div>
                    );
                  })}
                </div>
